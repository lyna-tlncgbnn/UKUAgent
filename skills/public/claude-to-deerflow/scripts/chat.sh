#!/usr/bin/env bash
# chat.sh — Send a message to UKUBot and collect the streaming response.
#
# Usage:
...
#
# Note: DEERFLOW_* variable names are retained for backward compatibility.
...
if [ "$HTTP_CODE" = "000" ] || [ "$HTTP_CODE" -ge 400 ]; then
  echo "ERROR: UKUBot is not reachable at ${GATEWAY_URL} (HTTP ${HTTP_CODE})" >&2
  echo "Make sure UKUBot is running. Start it with: cd <ukubot-dir> && make dev" >&2
  exit 1
fi
...
if result_messages is not None:
    ...
    for event_type, data_str in events:
        if event_type == "error":
            print(f"ERROR from UKUBot: {data_str}", file=sys.stderr)
            sys.exit(1)
