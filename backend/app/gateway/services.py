...
async def start_agent_run(
    *,
    ...
    graph_input_raw: dict[str, Any] | None,
    request_config: dict[str, Any] | None,
    metadata: dict[str, Any] | None,
    ...
    """Create a run and launch the background agent task.

    This is the request-independent core used by HTTP run endpoints and
    scheduler-triggered tasks. HTTP validation remains in the routers; this
    function owns the shared runtime wiring and thread metadata synchronization.
    """
    ...
    try:
        record = await run_mgr.create_or_reject(
            ...
            kwargs={"input": graph_input_raw, "config": request_config},
    ...
    if business_store is not None and user_id is not None:
    ...
    graph_input = normalize_input(graph_input_raw)
    ...
    stream_modes = normalize_stream_modes(stream_mode)
