...
async def clear_memory(request: Request) -> MemoryResponse:
    ...
    try:
        current_user = require_current_user(request)
        memory_data = clear_memory_data(user_id=current_user.id)
...
async def delete_memory_fact_endpoint(fact_id: str, request: Request) -> MemoryResponse:
    ...
    try:
        current_user = require_current_user(request)
        memory_data = delete_memory_fact(fact_id, user_id=current_user.id)
...
async def import_memory(request: Request, payload: MemoryResponse) -> MemoryResponse:
    ...
    try:
        current_user = require_current_user(request)
        memory_data = import_memory_data(payload.model_dump(), user_id=current_user.id)
    except OSError as exc:
